...
export default function App() {
  ...
  async function decide(decision: "approved" | "denied") { if (!transaction) return; try { const liveWallet = Boolean(import.meta.env.VITE_CONTRACT_ADDRESS) && transaction.analysis.mode !== "mock"; if (liveWallet) { const signed = await signAndRecord(transaction.transactionHash, decision === "approved"); const decided = await submitDecision(transaction.id, decision, signed); setTransaction(await recordOnChain(decided.id, signed.blockchainTxHash)); } else { setTransaction(await submitDecision(transaction.id, decision)); } } catch (err) { setError(err instanceof Error ? err.message : "Decision failed"); } }
